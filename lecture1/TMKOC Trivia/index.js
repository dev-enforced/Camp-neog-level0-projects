...
log(chalk.bold.cyan("\nGame Info: TMKOC or Taarak Mehta ka Ooltah Chashmah is one of the longest running Indian Hindi Language television sitcoms. \n Click on the link to watch the episodes :"+moreEpisodes))
...
function evaluation(questionAsked,answerGiven){
  ...
  if(userAnswer.toUpperCase()===answerGiven.toUpperCase()){
    log(chalk.green("Yipppeee!!! "+userAnswer+" is the right answer"))
    score+=4;
  }else{
    log(chalk.red("Unfortunately your answer is incorrect."))
    score-=2;
...
if(correctAnswered==3||correctAnswered==2||correctAnswered==1){
  log(chalk.yellow("Congratulations you are among the high Scorers"))
}
